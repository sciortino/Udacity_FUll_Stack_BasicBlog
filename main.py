...
def valid_pw(name, pw, h):
    split_str = h.split(",")[1]
    return True if h == make_pw_hash_withpinch(name, pw, split_str) else False
...
def get_User_id(self):
    ...
    return int(cookie.split('|')[0]) if cookie else None
    ...
    def post(self):
        ...
        if logged_in:
            ...
            if subject and blog:
                ...
                self.render("post.html", edit_type="New Post",
                            subject=subject, blog=blog, blog_err=blog_err)
        else:
            self.redirect("/login")
        ...
        if not user or not pw == verify or not pw:
            ...
            if q:
                ...
                self.response.headers.add_header(
                    'Set-Cookie', str("user_id=%s; Path=/" % cookie))
